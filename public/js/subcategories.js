...
const inputSubCategory = document.querySelector('input[name="searchSubCategory"]');
...
function addItemsToTable(items) {
    const tableCategories = document.querySelector('#subcategories');
...
if (inputSubCategory) {
    ...
    inputSubCategory.addEventListener('input', (e) => {
        clearTimeout(timer); // Limpiar el temporizador anterior
        timer = setTimeout(async () => {
            const search = e.target.value.toLowerCase().trim();

            if(search.length == 0){
                const response = await fetchFunction(search, `${url}/index.php/ipanel/searchAllSubCategories`);
            ...
            if (search.length > 0) {
                const response = await fetchFunction(search, `${url}/index.php/ipanel/searchSubCategories`);
                if(response.length > 0) {
                    addItemsToTable(response);
                }
            }
...
const formNewSubcategory = document.querySelector('#formNewSubcategory');

if(formNewSubcategory){
    formNewSubcategory.addEventListener('submit', async (e) => {
        e.preventDefault();
        ...
        confirm.addEventListener('click', async (e) => {
            e.preventDefault();
            myAlert.close();
            const response = await fetchFunctionPost(data, `${url}/index.php/ipanel/newSubcategory`);
            if(response){
...
if(tableCategories){
    tableCategories.addEventListener('click', async (e) => {
        ...
        if(target.closest('a') && target.closest('a').classList.contains('table-edit')){
            ...
            window.location.href = `${url}/index.php/ipanel/editSubcategory/?id=${data.id}&nombre=${data.nombre}`;
        }
        ...
        if(target.closest('a') && target.closest('a').classList.contains('table-trash')){
            ...
            confirm.addEventListener('click', async (e) => {
                e.preventDefault();
                myAlert.close();
                const response = await fetchFunctionPost(id, `${url}/index.php/ipanel/deleteSubcategory`);
                if(response){
...
const formEditCategory = document.querySelector('#formEditSubcategory');
    ...
    formEditCategory.addEventListener('submit', async (e) => {
        ...
        if(data.categoryName.length > 0){
            ...
            confirm.addEventListener('click', async (e) => {
                e.preventDefault();
                myAlert.close();
                const response = await fetchFunctionPost(data, `${url}/index.php/ipanel/updateSubcategory`);
                if(response){
                    myAlert.showNotification('Subcategoría actualizada', 1);
                    setTimeout(() => {
                        window.location.href = `${url}/index.php/ipanel/subcategories`;
                    }, 2200);
                }else{
                    myAlert.showNotification('Ups algo ocurrió, vuelve a intentarlo ', 3);
                }
